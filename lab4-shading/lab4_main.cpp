...
void initFullScreenQuad()
{
	...
	{
		// Defining quad vertices in normalized device coordinates
		// Each vertex is a 2D position in the range [-1, 1].
		...
		// we generate and bind a Vertex Array Object (VAO)
		GLuint screenQuadVBO;
		glGenVertexArrays(1, &fullScreenQuadVAO);
		glBindVertexArray(fullScreenQuadVAO);

		// then generate and bind a Vertex Buffer Object (VBO)
		...
		// specify the layout of the vertex data (only positions, 2 floats per vertex)
		glEnableVertexAttribArray(0); // Attribute 0 in the vertex shader
...
void drawFullScreenQuad()
{
	...
	///////////////////////////////////////////////////////////////////////////
	// Task 4.2
	GLboolean previous_depth_state; // save current depht state
	glGetBooleanv(GL_DEPTH_TEST, &previous_depth_state); // get currect state of depth test
	glDisable(GL_DEPTH_TEST); // disable depth test
	glBindVertexArray(fullScreenQuadVAO);
	glDrawArrays(GL_TRIANGLES, 0, 6); // 6 vertices, 2 triangles

	// restore depth test state
	if (previous_depth_state == GL_TRUE) {
