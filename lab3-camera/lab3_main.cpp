...
void initialize()
{
	...
	// TASK 1: move car by 5 units along Y axis
	carModelMatrix = glm::translate(mat4(1.0f), vec3(0, 5.0f, 0));
}
...
void display()
{
	...
	// TASK 4: Calculate the camera's right and up vectors based on the current direction (-Z axis, computing cameraRight x, 
	// and y cameraUp
	vec3 cameraRight = normalize(cross(cameraDirection, worldUp));
	vec3 cameraUp = normalize(cross(cameraRight, cameraDirection));

	// Build the camera base matrix
	...
	// Set up the view matrix with camera rotation and translation
	mat4 viewMatrix = cameraRotation * glm::translate(-cameraPosition);
	...
	// TASK 3: Calculate the position of the second car along the circular track
	...
	mat4 secondCarModelMatrix = t * t2 * r;

	// TASK 3: We set up model-view-projection for the second car
	mat4 secondCarMVP = projectionMatrix * viewMatrix * secondCarModelMatrix;
	glUniformMatrix4fv(mvploc, 1, false, &secondCarMVP[0].x);
	glUniformMatrix4fv(mloc, 1, false, &secondCarModelMatrix[0].x);

	// TASK 3: Render the second car
	render(carModel); // Reusing carModel for the second car
...
bool handleEvents(void)
{
	...
	while(SDL_PollEvent(&event))
	{
		...
		if(event.type == SDL_MOUSEMOTION && g_isMouseDragging && !(ImGui::GetIO().WantCaptureMouse))
		{
			...
			// TASK 4: replaced mouse motion event with code below
	...
	if (!ImGui::GetIO().WantCaptureKeyboard)
	{
		...
		if (state[SDL_SCANCODE_LEFT]) 	// TASK 2: car rotation
		{
			R = glm::rotate(rotateSpeed * deltaTime, glm::vec3(0, 1, 0)) * R;
		}
		if (state[SDL_SCANCODE_RIGHT]) 	// TASK 2: car rotation
		{
			R = glm::rotate(-rotateSpeed * deltaTime, glm::vec3(0, 1, 0)) * R;
		}

		// Camera translation controls (move forward and backward)
		if (state[SDL_SCANCODE_W]) {
			cameraPosition += cameraDirection * speed * deltaTime; // Move forward
		}
		if (state[SDL_SCANCODE_S]) {
			cameraPosition -= cameraDirection * speed * deltaTime; // Move backward
		}
	}


	carModelMatrix = T * R; //previously from  carModelMatrix = T;
