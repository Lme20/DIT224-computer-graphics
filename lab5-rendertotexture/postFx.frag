...
// Helper function to create a mosaic effect using the floor approach
vec2 mosaic(vec2 inCoord)
{
    float mosaicSize = 22.0; // Size of each mosaic bucket
    return floor(inCoord / mosaicSize) * mosaicSize; // Calculate the top-left corner of the bucket
}
...
vec3 toSepiaTone(vec3 rgbSample)
{
	...
	vec3 yiq = vec3(dot(yiqTransform0, rgbSample), 0.1, 0.0);
