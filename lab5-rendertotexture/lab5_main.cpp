...
GLuint verticalBlurShader = 0, horizontalBlurShader = 0, cutoffShader = 0; // Task 6
...
struct FboInfo
{
	...
	FboInfo(int w, int h)
	{
		...
		// glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT); task 3
		...
		///////////////////////////////////////////////////////////////////////
		// Task 1
		glGenFramebuffers(1, &framebufferId); // generate framebuffer
		...
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, colorTextureTarget, 0); // bind the texture as color attachment 0
		glDrawBuffer(GL_COLOR_ATTACHMENT0); // set the color attachment 0 as the draw buffer

		// bind the texture as depth attachment (to the currently bound framebuffer)
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, depthBuffer, 0); // bind the texture as depth attachment
...
void initialize()
{
	...
	///////////////////////////////////////////////////////////////////////////
	// Setup Framebuffers - TASK 1
	///////////////////////////////////////////////////////////////////////////
	int w, h;
	SDL_GetWindowSize(g_window, &w, &h); // get the window size
	const int numFbos = 5; // number of framebuffers
...
void display()
{
	...
	///////////////////////////////////////////////////////////////////////////
	// Task 2
	FboInfo& securityFB = fboList[0]; // get the first framebuffer object
	glBindFramebuffer(GL_FRAMEBUFFER, securityFB.framebufferId); // bind the framebuffer

	glViewport(0, 0, securityFB.width, securityFB.height); // set the viewport to the framebuffer size
	glClearColor(0.2, 0.2, 0.8, 1.0); // set the clear color (background color)
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); // clear the color and depth buffer

	// draw the scene from the security camera view
	drawScene(securityCamViewMatrix, securityCamProjectionMatrix);// using both shaderProgram and backgroundProgram

	labhelper::Material& screen = landingpadModel->m_materials[8]; // get the material of the landing pad
	screen.m_emission_texture.gl_id = securityFB.colorTextureTarget; // set the emission texture to the color texture of the framebuffer
	...
	FboInfo& cameraFB = fboList[1]; // get the next framebuffer object
	glBindFramebuffer(GL_FRAMEBUFFER, cameraFB.framebufferId); // bind the framebuffer
	glViewport(0, 0, w, h);
	...
	FboInfo& verticalBlurFbo = fboList[4]; // get the next framebuffer object
	...
	if (currentEffect == PostProcessingEffect::Separable_blur)
	{
		...
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, horizontalBlurFbo.colorTextureTarget); // Input from horizontal blur FBO
		labhelper::setUniformSlow(verticalBlurShader, "inputTexture", 0);
	...
	glViewport(0, 0, w, h); // set the viewport to the framebuffer size
	glClearColor(0.2f, 0.2f, 0.8f, 1.0f); // set the clear color (background color)
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); // clear the color and depth buffer

	// 2. Set postFxShader as active
	glUseProgram(postFxShader); // set the postFxShader as active program

	// 3. Bind the framebuffer to texture unit 0
	glActiveTexture(GL_TEXTURE0); // set the active texture unit to 0
	glBindTexture(GL_TEXTURE_2D, cameraFB.colorTextureTarget); // bind the color texture of the camera framebuffer to the active texture unit

	// Task 6: Bind the horizontal blur texture to texture unit 1
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, verticalBlurFbo.colorTextureTarget);

	// 4. Draw a quad over the entire viewport
	labhelper::drawFullScreenQuad(); // draw a full screen quad
