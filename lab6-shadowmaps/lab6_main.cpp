...
bool useSpotLight = true; // Task 6 - previously set to false
...
int shadowMapClampMode = ClampMode::Border; // Task 5 - previously set to Edge
bool shadowMapClampBorderShadowed = false;
bool usePolygonOffset = true; // Task 3 - previously set to false
bool useSoftFalloff = false;
bool useHardwarePCF = false;
float polygonOffset_factor = 1.5f;
float polygonOffset_units = 2.0;
...
void initialize()
{
	...
	// Task 8 - Configure shadow map depth texture for depth comparison in shadow mapping
               ...
               const mat4& lightProjectionMatrix)
{
	...
	// Shadow map - Task 2
	glActiveTexture(GL_TEXTURE10); // Texture unit 10 is for shadow maps.
	glBindTexture(GL_TEXTURE_2D, shadowMapFB.depthBuffer); // Bind the shadow map.
...
void display(void)
{
	...
	///////////////////////////////////////////////////////////////////////////
	// Task 1 - resize framebuffer if width or height is not equal to shadowMapResolution
	...
	// Task 5 - set shadow map clamp mode
	...
	// Task 9 - set shadow map texture parameters - GL_LINEAR for PCF
	if (useHardwarePCF) {
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); // task 9 - changed from GL_NEAREST
	...
	// Draw Shadow Map
	///////////////////////////////////////////////////////////////////////////
	// bind and clear framebuffer + set to viewport, then draw the scene with simple shader program
	glBindFramebuffer(GL_FRAMEBUFFER, shadowMapFB.framebufferId); // bind the framebuffer
	glViewport(0, 0, shadowMapFB.width, shadowMapFB.height); // set viewport to framebuffer size
	glClearColor(1.0f, 1.0f, 1.0f, 1.0f); // clear color to white
	glClearDepth(1.0f); // clear depth to 1
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); // clear color and depth buffer

	// task 3 - set polygon offset
	if (usePolygonOffset) {
		glEnable(GL_POLYGON_OFFSET_FILL);
		glPolygonOffset(polygonOffset_factor, polygonOffset_units);
	}

	// draw the scene with the depth shader program
	drawScene(simpleShaderProgram, lightViewMatrix, lightProjMatrix, lightViewMatrix, lightProjMatrix);

	// task 3 - disable polygon offset
	if (usePolygonOffset) {
